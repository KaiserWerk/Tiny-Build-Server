...
func (bd golangBuildDefinition) runBenchmarkTests(smessageCh chan string) error {
...
func (bd golangBuildDefinition) buildArtifact(messageCh chan string, projectDir string) (string, error) {
	...
	buildCommand := `build -o "<output>" -a -v -work -x -ldflags "-s -w -X main.versionDate=` + dateStr + `" <input>`
